...
function updateCommentConfigUI() {
    ...
    const openEndedCheckbox = document.getElementById('openEndedComment') as HTMLInputElement;

    if (lengthSlider) {
        const lengthValues = ['veryShort', 'short', 'medium', 'long', 'veryLong'];
...
function updateReplyConfigUI() {
    const keepShortCheckbox = document.getElementById('keepShort') as HTMLInputElement;
    const openEndedCheckbox = document.getElementById('openEndedReply') as HTMLInputElement;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    lengthSlider?.addEventListener('input', (e) => {
        ...
        const lengthLabels = ['Very Short', 'Short', 'Medium', 'Long', 'Very Long'];
        if (lengthValue) lengthValue.textContent = lengthLabels[parseInt(slider.value)];
    });

    lengthSlider?.addEventListener('change', async (e) => {
        const slider = e.target as HTMLInputElement;
        const lengths = ['veryShort', 'short', 'medium', 'long', 'veryLong'];
        commentConfig.length = lengths[parseInt(slider.value)] as CommentConfig['length'];
    ...
    document.getElementById('openEndedComment')?.addEventListener('change', async (e) => {
    ...
    document.getElementById('keepShort')?.addEventListener('change', async (e) => {
        const checkbox = e.target as HTMLInputElement;
    ...
    document.getElementById('openEndedReply')?.addEventListener('change', async (e) => {
        const checkbox = e.target as HTMLInputElement;
